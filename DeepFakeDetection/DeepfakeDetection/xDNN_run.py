...
# print ("###################### Parameters ####################")
# print("x:", x)  # Debug: Print the whole x to check its structure
# print("x['xDNNParms']:", x.get('xDNNParms'))  # Debug: Print x['xDNNParms']
# print("x['xDNNParms']['Parameters']:", x.get('xDNNParms', {}).get('Parameters'))  # Debug: Print x['xDNNParms']['Parameters']

Prototypes = x.get('xDNNParms', {}).get('Parameters', [])
total_prototypes = 0
# print('Number of Training Data Samples:', len(X_train))

for i in range(len(Prototypes)):
    # print(f"Prototypes[{i}] content: {Prototypes[i]}")  # Debug: Print the content of each prototype
    if Prototypes[i] is not None:
        class_prototypes = len(Prototypes[i].get('Prototype', []))  # Use .get() to handle missing keys
        total_prototypes += class_prototypes
        # print("Class", sub_folders[i], ":", class_prototypes)
    else:
      pass
        # print(f"Prototypes[{i}] is None")
...
# import numpy as np
# data = Prototypes[1]["Centre"]
# labels = np.array([])
# i=1
# for i in range(1,len(Prototypes)):
#     if i != 0:
#         data = np.concatenate((data[1:], Prototypes[i]["Centre"]))
